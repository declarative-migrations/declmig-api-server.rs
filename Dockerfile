...
#
# declmig-api-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/declarative-migrations/declmig-api-server:dev --load .
#
#   # This crate resolves 1 dependency from git remotes. If any of them is
...
#     -t ghcr.io/declarative-migrations/declmig-api-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/declarative-migrations/declmig-api-server:dev .
#
#   `just docker-build` / `just docker-run` / `just docker-push` wrap all three.
#
# DEPENDENCIES — 3 crate(s) come from git remotes (Cargo.toml:
# declmig-lib-core, ores-transport, k8s-web-api-data-plane) and 0 from zed-pkg (.zpkg.toml: none).
...
#     ghcr.io/declarative-migrations/declmig-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin declmig-api-server; \
    strip target/release/declmig-api-server; \
    cp target/release/declmig-api-server /usr/local/bin/declmig-api-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/declmig-api-server /usr/local/bin/declmig-api-server
ENV DECLMIG_API_BIND=0.0.0.0:8080
EXPOSE 8080
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/declmig-api-server"]
...
COPY --from=builder /usr/local/bin/declmig-api-server /usr/local/bin/declmig-api-server
...
CMD ["/usr/local/bin/declmig-api-server"]
