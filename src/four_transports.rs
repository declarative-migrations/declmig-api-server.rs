#![forbid(unsafe_code)]
//! Serving the four avenues from `declmig-api-server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `declmig-lib-core` |
...
pub const SERVICE_SLUG: &str = "declmig";

/// Environment prefix for every variable this service reads.
pub const ENV_PREFIX: &str = "DECLMIG";
...
///
/// Bind `DECLMIG_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `DECLMIG_NATS_URL`.
///
/// # Errors
