...
pub struct CliEnvValues {
    ...
    pub declmig_api_bind: Option<String>,
    pub declmig_api_tcp_bind: Option<String>,
    pub declmig_nats_url: Option<String>,
...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        api_base: lookup("DECLMIG_API_BASE").filter(|value| !value.is_empty()),
        config: lookup("DECLMIG_CONFIG").filter(|value| !value.is_empty()),
        declmig_api_bind: lookup("DECLMIG_API_BIND").filter(|value| !value.is_empty()),
        declmig_api_tcp_bind: lookup("DECLMIG_API_TCP_BIND").filter(|value| !value.is_empty()),
        declmig_nats_url: lookup("DECLMIG_NATS_URL").filter(|value| !value.is_empty()),
        json: lookup("DECLMIG_JSON").map(|raw| parse_bool(Some(raw), false)),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let api_base = pick(&["DECLMIG_API_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = api_base {
        out.insert("DECLMIG_API_BASE".to_string(), value);
    }
    let config = pick(&["DECLMIG_CONFIG"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = config {
        out.insert("DECLMIG_CONFIG".to_string(), value);
    }
    let declmig_api_bind = pick(&["DECLMIG_API_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = declmig_api_bind {
        out.insert("DECLMIG_API_BIND".to_string(), value);
    }
    let declmig_api_tcp_bind = pick(&["DECLMIG_API_TCP_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = declmig_api_tcp_bind {
        out.insert("DECLMIG_API_TCP_BIND".to_string(), value);
    }
    let declmig_nats_url = pick(&["DECLMIG_NATS_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = declmig_nats_url {
        out.insert("DECLMIG_NATS_URL".to_string(), value);
    }
    let json = pick(&["DECLMIG_JSON"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = json {
        out.insert("DECLMIG_JSON".to_string(), value);
    }
    Ok(out)
}
